...
def add_critical_inspection_items():
    ...
    # إضافة فئة "الهيكل الخارجي" إذا كانت موجودة
    try:
        exterior_category = CarInspectionCategory.objects.filter(name__icontains="الهيكل الخارجي").first()
        if exterior_category:
            important_category_objects.append(exterior_category)
    except Exception as e:
        print(f"❌ خطأ في العثور على فئة 'الهيكل الخارجي': {str(e)}")
    ...
    critical_items = {
        ...
        "المحرك": ["المحرك", "زيت المحرك", "نظام التبريد", "البطارية"],
        "نظام الفرامل": ["الفرامل الأمامية", "الفرامل الخلفية", "فرامل اليد", "سائل الفرامل"],
        "الإطارات": ["الإطارات الأمامية", "الإطارات الخلفية", "ضغط الهواء", "حالة المداس"],
        "أنظمة السلامة": ["الوسائد الهوائية", "أحزمة الأمان", "نظام ABS", "نظام ESP"],
    }
