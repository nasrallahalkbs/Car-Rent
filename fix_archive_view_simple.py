...
def fix_admin_archive_view():
    ...
    old_render_line = "    return render(request, 'admin/archive/archive_direct.html', context)"
    new_render_line = "    return render(request, 'admin/archive/simple_archive.html', context)"
    ...
    if old_render_line in content:
        ...
        print(f"تم تحديث ملف {admin_views_path} لاستخدام قالب الأرشيف البسيط")
        return True
        ...
        if "def admin_archive(" in lines[i]:
            ...
            for j in range(i, min(i + 100, len(lines))):
                if "return render" in lines[j] and "admin/archive/" in lines[j]:
                    # استبدال اسم القالب
                    lines[j] = "    return render(request, 'admin/archive/simple_archive.html', context)"
                    updated = True
                    break
...
def main():
    """تنفيذ الإصلاحات"""
    print("بدء إصلاح عرض الأرشيف باستخدام قالب مبسط...")
