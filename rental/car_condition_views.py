...
def complete_car_inspection_create(request):
    ...
    most_important_item_ids = [
        # أجزاء السيارة الرئيسية - العناصر الأصلية
        2,    # غطاء المحرك
        3,    # الأبواب
        5,    # المصدات
        8,    # الزجاج الأمامي
        # أجزاء السيارة الرئيسية - العناصر الجديدة
        130,  # نظام الكمبيوتر الرئيسي
        131,  # عداد السرعة والمسافة
        132,  # المفتاح الذكي
        133,  # الأجهزة الكهربائية
        134,  # حساسات الأمان
        
        # المحرك ومكونات أسفل غطاء المحرك - العناصر الأصلية
        103,  # المحرك
        104,  # زيت المحرك
        105,  # نظام التبريد
        101,  # البطارية
        # المحرك ومكونات أسفل غطاء المحرك - العناصر الجديدة
        135,  # مضخة المياه
        136,  # مضخة الزيت
        137,  # خراطيم التبريد
        138,  # جهاز الإشعال
        ...
        # الإطارات والعجلات - العناصر الأصلية
        106,  # الإطارات
        107,  # العجلات
        110,  # قياس ضغط الإطارات
        111,  # حالة المداس
        # الإطارات والعجلات - العناصر الجديدة
        140,  # الإطار الاحتياطي
        141,  # تآكل الإطارات
        142,  # توازن العجلات
        143,  # محاذاة العجلات
        144,  # براغي العجلات
        
        # أنظمة السلامة والتشغيل - العناصر الأصلية
        108,  # نظام الفرامل
        109,  # الوسائد الهوائية
        112,  # نظام ABS
        113,  # أحزمة الأمان
        # أنظمة السلامة والتشغيل - العناصر الجديدة
        145,  # نظام ESP
        146,  # نظام ASR
        147,  # مساعد الفرامل
        148,  # كاميرا الرجوع للخلف
        149,  # حساسات الركن
    ]
    ...
    print(f"  - فئة (أجزاء السيارة الرئيسية): 9 عناصر")
    print(f"    * العناصر الأصلية: غطاء المحرك، الأبواب، المصدات، الزجاج الأمامي")
    print(f"    * العناصر الجديدة: نظام الكمبيوتر الرئيسي، عداد السرعة، المفتاح الذكي، الأجهزة الكهربائية، حساسات الأمان")
    
    print(f"  - فئة (المحرك ومكونات أسفل غطاء المحرك): 9 عناصر")
    print(f"    * العناصر الأصلية: المحرك، زيت المحرك، نظام التبريد، البطارية")
    print(f"    * العناصر الجديدة: مضخة المياه، مضخة الزيت، خراطيم التبريد، جهاز الإشعال، علبة المرشحات")
    
    print(f"  - فئة (الإطارات والعجلات): 9 عناصر")
    print(f"    * العناصر الأصلية: الإطارات، العجلات، قياس ضغط الإطارات، حالة المداس")  
    print(f"    * العناصر الجديدة: الإطار الاحتياطي، تآكل الإطارات، توازن العجلات، محاذاة العجلات، براغي العجلات")
    
    print(f"  - فئة (أنظمة السلامة والتشغيل): 9 عناصر")
    print(f"    * العناصر الأصلية: نظام الفرامل، الوسائد الهوائية، نظام ABS، أحزمة الأمان")
    print(f"    * العناصر الجديدة: نظام ESP، نظام ASR، مساعد الفرامل، كاميرا الرجوع للخلف، حساسات الركن")
