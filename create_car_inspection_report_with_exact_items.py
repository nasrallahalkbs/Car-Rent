...
"""
إنشاء تقرير فحص سيارة باستخدام العناصر المهمة/المكلفة/الحرجة الفعلية

هذا السكريبت يقوم بإنشاء تقرير فحص سيارة يستخدم العناصر الفعلية من النظام،
مع التركيز على العناصر المهمة/المكلفة/الحرجة فقط وفقًا للتصفية المستخدمة في واجهة النظام.
"""
...
def create_exact_car_inspection_report_with_filtered_items():
    """
    إنشاء تقرير فحص سيارة مطابق تماماً للنموذج الموجود
    باستخدام العناصر المهمة/المكلفة/الحرجة فقط
    """
    ...
    # العثور على عميل موجود
    try:
        customer = Customer.objects.first()
        print(f"تم العثور على عميل موجود: {customer.email}")
    except Exception as e:
        print(f"خطأ في العثور على عميل: {str(e)}")
        return
    ...
    try:
        report = CarConditionReport.objects.create(
            ...
            date=timezone.now(),
            condition='good',  # حالة السيارة جيدة
            ...
            damages='خدوش بسيطة في الصدام الأمامي والخلفي. صوت غير طبيعي في المحرك. تآكل في فحمات الفرامل.',
            damage_cause='الاستخدام العادي والاستهلاك',
    ...
    # إنشاء توقيع للعميل والموظف
    try:
        # ربط التوقيعات بتفاصيل التقرير
        report.customer_name = f"{customer.first_name} {customer.last_name}"
        report.employee_name = admin_user.profile.name
        report.save()
        print(f"✅ تم إنشاء توقيع للعميل: {report.customer_name}")
        print(f"✅ تم إنشاء توقيع للموظف: {report.employee_name}")
    except Exception as e:
    ...
    print(f"حالة السيارة: {report.get_condition_display()}")
    print(f"مستوى الوقود: {report.get_fuel_level_display()}")
    print(f"الملاحظات: {report.notes}")
    print(f"الأعطال: {report.damages}")
    print(f"سبب العطل: {report.damage_cause}")
...
if __name__ == "__main__":
    ...
    create_exact_car_inspection_report_with_filtered_items()